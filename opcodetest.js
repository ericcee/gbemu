...
function op3A(){ // LD A,(HL-)
	...
	writeMem(0, 0x3A);
    reg[HL] = 0x1155;
    writeMem(reg[HL], 0xFD);
	var cpul = decode[readMem(0)]();
    
    var passes = [];
    passes.push(reg[HL] == 0x1154);
    passes.push(getByteRegister(A) == 0xFD);
    passes.push(reg[PC] == 1);
    passes.push(cpul == 8);
...
function op3C(){ // INC A
    ...
    var passes = [reg[PC]==1, cpul == 4, getByteRegister(A) == 1, getFlag(_Z) == 0, getFlag(_N) == 0, getFlag(_H) == 0, oldCarry == getFlag(_C)];
    reg[PC] = 0;
...
function op3D(){ // DEC A
    ...
    passes.push(getFlag(_C) == c);
    passes.push(getFlag(_H) == 0);
    passes.push(getByteRegister(A) == 0xFE);
...
function op3E(){ // LD A,d8
    ...
    passes.push(cpul == 8);
    passes.push(reg[PC] == 2);
    passes.push(getByteRegister(A) == 0x12);
...
function LDxy(r1, r2){
    reset();
    var cpul = decode[readMem(0)]();
    var r1v = getByteRegister(r1v);
    var r2v = getByteRegister(r2v);
...
function LDxa(r1, r16){
    reset();
    reg[r16] = 0x1234;
    writeMem(reg[r16], 0xDF);
    
    var cpul = decode[readMem(0)]();
    
    var passes = [];
    passes.push(cpul == 8);
    passes.push(getByteRegister(r1) == readMem(reg[r16]));
    passes.push(reg[PC] == 1);
...
function LDax(r16, r1){
    reset();
    reg[r16] = 0x1235;
    if(r1 != H && r1 != L)setByteRegister(r1, 0xDF);
    
    var cpul = decode[readMem(0)]();
    
    var passes = [];
    passes.push(cpul == 8);
    passes.push(readMem(0x1235) == getByteRegister(r1));
    passes.push(reg[PC] == 1);
    
    if(r1 != H && r1 != L){}
    else {
        console.log(readMem(reg[r16]).toString(16));
        console.log(getByteRegister(r1).toString(16));
        console.log(readMem(0x1235) == getByteRegister(r1));
    }
...
function op40(){ // LD B,B
	writeMem(0, 0x40);
    return LDxy(B,B);
}
function op41(){ // LD B,C
	writeMem(0, 0x41);
	return LDxy(B,C);
}
function op42(){ // LD B,D
	writeMem(0, 0x42);
	return LDxy(B,D);
}
function op43(){ // LD B,E
	writeMem(0, 0x43);
	return LDxy(B,E);
}
function op44(){ // LD B,H
	writeMem(0, 0x44);
	return LDxy(B,H);
}
function op45(){ // LD B,L
	writeMem(0, 0x45);
	return LDxy(B,L);
}
function op46(){ // LD B,(HL)
	writeMem(0, 0x46);
	return LDxa(B, HL);
}
function op47(){ // LD B,A
	writeMem(0, 0x47);
	return LDxy(B,A);
}
function op48(){ // LD C,B
	writeMem(0, 0x48);
	return LDxy(C,B);
}
function op49(){ // LD C,C
	writeMem(0, 0x49);
	return LDxy(C,C);
}
function op4A(){ // LD C,D
	writeMem(0, 0x4A);
	return LDxy(C,D);
}
function op4B(){ // LD C,E
	writeMem(0, 0x4B);
	return LDxy(C,E);
}
function op4C(){ // LD C,H
	writeMem(0, 0x4C); 
	return LDxy(C,H);
}
function op4D(){ // LD C,L
	writeMem(0, 0x4D);
	return LDxy(C,L);
}
function op4E(){ // LD C,(HL)
	writeMem(0, 0x4E);
	return LDxa(C, HL);
}
function op4F(){ // LD C,A
	writeMem(0, 0x4F);
	return LDxy(C,A);
}
function op50(){ // LD D,B
	writeMem(0, 0x50);
	return LDxy(D,B);
}
function op51(){ // LD D,C
	writeMem(0, 0x51);
	return LDxy(D,C);
}
function op52(){ // LD D,D
	writeMem(0, 0x52);
	return LDxy(D,D);
}
function op53(){ // LD D,E
	writeMem(0, 0x53);
	return LDxy(D,E);
}
function op54(){ // LD D,H
	writeMem(0, 0x54);
	return LDxy(D,H);
}
function op55(){ // LD D,L
	writeMem(0, 0x55);
	return LDxy(D,L);
}
function op56(){ // LD D,(HL)
	writeMem(0, 0x56);
	return LDxa(D,HL);
}
function op57(){ // LD D,A
	writeMem(0, 0x57);
	return LDxy(D,A);
}
function op58(){ // LD E,B
	writeMem(0, 0x58);
	return LDxy(E,B);
}
function op59(){ // LD E,C
	writeMem(0, 0x59);
	return LDxy(E,C);
}
function op5A(){ // LD E,D
	writeMem(0, 0x5A);
	return LDxy(E,D);
}
function op5B(){ // LD E,E
	writeMem(0, 0x5B);
	return LDxy(E,E);
}
function op5C(){ // LD E,H
	writeMem(0, 0x5C);
	return LDxy(E,H);
}
function op5D(){ // LD E,L
	writeMem(0, 0x5D);
	return LDxy(E,L);
}
function op5E(){ // LD E,(HL)
	writeMem(0, 0x5E);
	return LDxa(E,HL);
}
function op5F(){ // LD E,A
	writeMem(0, 0x5F);
	return LDxy(E,A);
}
function op60(){ // LD H,B
	writeMem(0, 0x60);
	return LDxy(H,B);
}
function op61(){ // LD H,C
	writeMem(0, 0x61);
	return LDxy(H,C);
}
function op62(){ // LD H,D
	writeMem(0, 0x62);
	return LDxy(H,D);
}
function op63(){ // LD H,E
	writeMem(0, 0x63);
	return LDxy(H,E);
}
function op64(){ // LD H,H
	writeMem(0, 0x64);
	return LDxy(H,H);
}
function op65(){ // LD H,L
	writeMem(0, 0x65);
	return LDxy(H,L);
}
function op66(){ // LD H,(HL)
	writeMem(0, 0x66);
	return LDxa(H,HL);
}
function op67(){ // LD H,A
	writeMem(0, 0x67);
	return LDxy(H,A);
}
function op68(){ // LD L,B
	writeMem(0, 0x68);
	return LDxy(L,B);
}
function op69(){ // LD L,C
	writeMem(0, 0x69);
	return LDxy(L,C);
}
function op6A(){ // LD L,D
	writeMem(0, 0x6A);
	return LDxy(L,D);
}
function op6B(){ // LD L,E
	writeMem(0, 0x6B);
	return LDxy(L,E);
}
function op6C(){ // LD L,H
	writeMem(0, 0x6C);
	return LDxy(L,H);
}
function op6D(){ // LD L,L
	writeMem(0, 0x6D);
	return LDxy(L,L);
}
function op6E(){ // LD L,(HL)
	writeMem(0, 0x6E);
	return LDxa(L,HL);
}
function op6F(){ // LD L,A
	writeMem(0, 0x6F);
	return LDxy(L,A);
}
function op70(){ // LD (HL),B
	writeMem(0, 0x70);
	return LDax(HL,B);
}
function op71(){ // LD (HL),C
	writeMem(0, 0x71);
	return LDax(HL,C);
}
function op72(){ // LD (HL),D
	writeMem(0, 0x72);
	return LDax(HL,D);
}
function op73(){ // LD (HL),E
	writeMem(0, 0x73);
	return LDax(HL,E);
}
function op74(){ // LD (HL),H
	writeMem(0, 0x74);
	return LDax(HL,H);
}
function op75(){ // LD (HL),L 
	writeMem(0, 0x75);
	return LDax(HL,L);
}
function op76(){ // HALT
...
function op77(){ // LD (HL),A
	writeMem(0, 0x77);
	return LDax(HL,A);
}
function op78(){ // LD A,B
	writeMem(0, 0x78);
	return LDxy(A,B);
}
function op79(){ // LD A,C
	writeMem(0, 0x79);
	return LDxy(A,C);
}
function op7A(){ // LD A,D
	writeMem(0, 0x7A);
	return LDxy(A,D);
}
function op7B(){ // LD A,E
	writeMem(0, 0x7B);
	return LDxy(A,E);
}
function op7C(){ // LD A,H
	writeMem(0, 0x7C);
	return LDxy(A,H);
}
function op7D(){ // LD A,L
	writeMem(0, 0x7D);
	return LDxy(A,L);
}
function op7E(){ // LD A,(HL)
	writeMem(0, 0x7E);
	return LDxa(A,HL);
}
function op7F(){ // LD A,A
	writeMem(0, 0x7F);
	return LDxy(A,A);
}
